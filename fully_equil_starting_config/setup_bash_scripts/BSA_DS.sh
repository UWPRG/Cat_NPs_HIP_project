...
for i in 1 2 5 10 ; do	
	...
	for j in 1 2 3 ; do 
		if [ ! -d em_ions/ ] ; then 	
			...
			cp ../../../DS12merHMR_posre.itp em_ions
			cp ../../../HMR_DS12mer.itp em_ions
			cp ../../../DS_12mer.pdb em_ions		
			cp ../../../K.pdb em_ions
			cp ../../../CL.pdb em_ions
			cp ../../../nvt.mdp nvt_eq
			cp ../../../npt.mdp npt_eq
...
#
# BSA/Dextran Sulphate/K/CL in water system

seed -1

tolerance 2.0
filetype pdb
output BSA_DS_MR${i}t${j}.pdb
...
EOF1
                	        ...
                	        packmol < packmol.inp
                	        gmx editconf -f BSA_DS_MR${i}t${j}.pdb -c -box ${n_size} ${n_size} ${n_size} -o MR${i}_bsaDSt${j}.gro
                	        sed -i '24i\; Include DS parameters' topol.top 
				sed -i '25i\#include "HMR_DS12mer.itp"' topol.top
				echo "HMR_DS_12mer            ${i}" >> topol.top
				echo "K            ${pos_ions[counter]}" >> topol.top	
 			       	gmx solvate -cp MR${i}_bsaDSt${j}.gro -cs spc216.gro -o boxsolvDS_MR${i}t${j}.gro -p topol.top 
 			       	gmx grompp -f minim.mdp -c boxsolvDS_MR${i}t${j}.gro -p topol.top -maxwarn 1
...
EOF1
		...
		if [ ! -f nvt_eq/confout.gro ] ; then 
			...
			ln -s ../em_ions/DS12merHMR_posre.itp
			ln -s ../em_ions/HMR_DS12mer.itp
			ln -s /gscratch/pfaendtner/cnyambura/NEE_home_mox/BSA_Nano_Prep/amber99sb-star-ildnp.ff/
