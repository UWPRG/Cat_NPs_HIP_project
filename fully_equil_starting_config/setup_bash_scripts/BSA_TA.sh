...
for i in 32 64 128 256 ; do	
	...
	for j in 1 2 3 ; do 
		if [ ! -d em_ions/ ] ; then 	
			...
			cp ../../../tcl_posre.itp em_ions
			cp ../../../HMR_tcl.itp em_ions
			cp ../../../TCL.pdb em_ions		
			cp ../../../K.pdb em_ions
			cp ../../../CL.pdb em_ions
			cp ../../../nvt.mdp nvt_eq
...
#
# BSA/Taurocholic Acid/K/CL in water system

seed -1

tolerance 2.0
filetype pdb
output BSA_TA_MR${i}t${j}.pdb
...
EOF1
                	        ...
                	        packmol < packmol.inp
                	        gmx editconf -f BSA_TA_MR${i}t${j}.pdb -c -box ${n_size} ${n_size} ${n_size} -o MR${i}_bsaTAt${j}.gro
                	        sed -i '24i\; Include TA parameters' topol.top 
				sed -i '25i\#include "HMR_tcl.itp"' topol.top
				echo "TCL            ${i}" >> topol.top
				echo "K            ${pos_ions[counter]}" >> topol.top	
 			       	gmx solvate -cp MR${i}_bsaTAt${j}.gro -cs spc216.gro -o boxsolv_MR${i}t${j}.gro -p topol.top 
 			       	gmx grompp -f minim.mdp -c boxsolv_MR${i}t${j}.gro -p topol.top -maxwarn 1
...
EOF1
		...
		if [ ! -f nvt_eq/confout.gro ] ; then 
			...
			ln -s ../em_ions/tcl_posre.itp
			ln -s ../em_ions/HMR_tcl.itp
			ln -s /gscratch/pfaendtner/cnyambura/NEE_home_mox/BSA_Nano_Prep/amber99sb-star-ildnp.ff/
